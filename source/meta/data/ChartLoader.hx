...
class ChartLoader
{
	...
	public static function generateChartType(songData:SwagSong, ?typeOfChart:String = "FNF"):Array<Note>
	{
		...
		if(PlayState.SONG.song.toLowerCase() == 'monotone-attack'){
		...
		if(PlayState.SONG.song.toLowerCase() == 'voting-time' || PlayState.SONG.song.toLowerCase() == 'monotone-attack'){
				var daBeats:Int = 0; // Not exactly representative of 'daBeats' lol, just how much it has looped
			...
			for (section in rmtjData)
			{
					...
					for (songNotes in section.sectionNotes)
					{
						...
						if (gottaHitNote)
						{
						swagNote.noteType = 'Opponent 2 Sing';
						}
						swagNote.scrollFactor.set(0, 0);
						var susLength:Float = swagNote.sustainLength; // sus amogus
						...
						for (susNote in 0...Math.floor(susLength))
						{
							...
							unspawnNotes.push(sustainNote);
							sustainNote.mustPress = gottaHitNote;
							sustainNote.noteType = 'GF Sing';
							if (gottaHitNote)
							{
							sustainNote.noteType = 'Opponent 2 Sing';
							}
							/*
								This is handled in engine anyways, not necessary!
								if (sustainNote.mustPress)
